SELECT
    ...
    (
        'ps_mir_card_transactions',
        'Сверка по карточным транзакциям с документами АБС',
        DATE '2024-01-01',
        NULL,
        NULL,
        'DWH_RECON',
        ...
        [
            ...
            (
                'DET', 'Сверка по карточным транзакциям с документами АБС', 'reconciliation.ps_mir_card_transactions', NULL,
                 ARRAY
                 [
                    r_report_robot.md_rec_report_dataset_column('report_date', 'Дата отчёта', 'D', 'yyyy-mm-dd'),
                    ...
                    r_report_robot.md_rec_report_dataset_column('orig_system', 'Система', 'S', NULL),
                    ...
                    r_report_robot.md_rec_report_dataset_column('sum_settl_amount', 'sum(settl_amount)', 'N', '#,#0.00'),
                    r_report_robot.md_rec_report_dataset_column('sum_c_sum', 'sum(c_sum)', 'N', '#,#0.00'),
                    r_report_robot.md_rec_report_dataset_column('sum_amount_rrn', 'sum(settl_amount_rrn)', 'N', '#,#0.00'),
                    r_report_robot.md_rec_report_dataset_column('sum_nazn_rrn', 'sum(sum_nazn_rrn)', 'N', '#,#0.00'),
                    r_report_robot.md_rec_report_dataset_column('sum_sum_rrn', 'sum(sum_rrn)', 'N', '#,#0.00'),
