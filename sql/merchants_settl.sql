...
and t.id in (select txn_id from inv_invoice_entry_settl iie where iie.invoice_item_id  in 
             (select id from inv_invoice_item_settl iii where iii.invoice_id in 
              (select id from inv_invoice_settl ii where opening_date between to_date(date_trunc('month', current_date - interval '1 month')) -- to_date('2025-03-01') 
              and to_date(date_trunc('month', current_date) - interval '1 day')  --and to_date('2025-03-31')
    )))
group by ttype, agr.agr_number
union all
select 'reimb' as ttype, agr.agr_number, sum(C_SUM) as txn_amount 
from MAIN_DOCUM_
join acc_account_settl a on a.account_number = MAIN_DOCUM_.C_NUM_DT and a.audit_state <> 'R'
...
select
case when acc.C_MAIN_V_ID like '30232%' then 'debt_bank_end'
...
abs(sum(C_START_SUM + ( case when C_DT = 1 then -1*C_SUMMA else C_SUMMA end))) as txn_amount
 ...
 join acc_account_settl a on a.account_number = acc.C_MAIN_V_ID and a.audit_state <> 'R'
 join bo_agreement_settl agr on a.agreement_id = agr.id
where 1=1
...
from MAIN_DOCUM_
left join acc_account_settl a on a.account_number = MAIN_DOCUM_.C_NUM_KT and a.audit_state <> 'R'
left join bo_agreement_settl agr on a.agreement_id = agr.id
where STATE_ID = 'PROV'
