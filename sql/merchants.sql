...
rec_max_dates_start as (
    select COLLECTION_ID, max(C_DATE) as max_dt
    from records
    where C_DATE < to_date(date_trunc('month', current_date - interval '1 month')) --to_date('2025-03-01')--+1
    group by COLLECTION_ID
),
rec_max_ids_start as (
    ...
    where r.C_DATE < to_date(date_trunc('month', current_date - interval '1 month')) --to_date('2025-03-01')--+1
    group by r.COLLECTION_ID
)
...
select 
    ...
    and t.id in (select txn_id from inv_invoice_entry iie where iie.invoice_item_id  in 
                 (select id from inv_invoice_item iii where iii.invoice_id in 
                  (select id from inv_invoice ii where opening_date between (
                  to_date(date_trunc('month', current_date - interval '1 month')) --'2025-03-01'
...
and tt.code not in ('purchaseReimb','negativeReimb')
        ...
        (select id from inv_invoice_item iii where iii.invoice_id in 
        (select id from inv_invoice ii where opening_date between 
        to_date(date_trunc('month', current_date - interval '1 month')) --to_date('2025-03-01') 
        and  to_date(date_trunc('month', current_date) - interval '1 day') --to_date('2025-03-31')
    )))
group by ttype, agr.agr_number
union all
...
from main_docum_
join acc_account a on a.account_number = main_docum_.C_NUM_DT
join bo_agreement agr on a.agreement_id = agr.id
where STATE_ID = 'PROV'
...
where txn_class = 'F'
and tt.code  in ('C2BDISPUTE_DEBIT')
and t.id in (select txn_id from inv_invoice_entry iie where iie.invoice_item_id  in 
             (select id from inv_invoice_item iii where iii.invoice_id in 
              (select id from inv_invoice ii where opening_date 
              between to_date(date_trunc('month', current_date - interval '1 month')) -- to_date('2025-03-01') 
              and to_date(date_trunc('month', current_date) - interval '1 day') -- to_date('2025-03-31')
    )))
group by ttype, agr.agr_number
...
inner join bo_agreement agr on rcvr_agreement_id = agr.id
where txn_class = 'F'
and tt.code  in ('C2BDISPUTE_CREDIT')
and t.id in (select txn_id from inv_invoice_entry iie where iie.invoice_item_id  in 
    (select id from inv_invoice_item iii where iii.invoice_id in 
    (select id from inv_invoice ii where opening_date 
    between to_date(date_trunc('month', current_date - interval '1 month')) -- to_date('2025-03-01') 
...
abs(sum(C_START_SUM + ( case when C_DT = 1 then -1*C_SUMMA else C_SUMMA end))) as txn_amount
 ...
 join acc_account a on a.account_number = acc.C_MAIN_V_ID
 join bo_agreement agr on a.agreement_id = agr.id
where 1=1
...
from MAIN_DOCUM_
left join acc_account a on a.account_number = MAIN_DOCUM_.C_NUM_KT
left join bo_agreement agr on a.agreement_id = agr.id
where STATE_ID = 'PROV'
